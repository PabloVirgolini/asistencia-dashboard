...
5. **AISLAMIENTO DE COMPONENTES UI (Testability)**: **SIEMPRE** debes extraer la lógica de datos (tRPC/BD) a *Custom Hooks* para aislar los componentes visuales. Debes probarlos mediante Vitest mockeando dichos hooks para no consultar la base de datos real en pruebas de interfaz. Además, al crear o modificar un Custom Hook, **DEBES** revisar los tests existentes que lo usen, ajustar los mocks si su firma cambió, y ejecutar `pnpm test` para garantizar que no haya regresiones.
