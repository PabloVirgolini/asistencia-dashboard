...
### [2026-06-13] - Script de Inicialización (Seed DB) y Robustez de Sesión JWT
- **Avance Arquitectónico (Seeder):** Se creó el script `seed_db.cjs` para automatizar la creación e inicialización de la base de datos `data2.db` desde cero. Esto permite que nuevos clones o entornos de pruebas cuenten con las 11 tablas del esquema relacional y un set de datos de prueba pre-cargado para el 13 de Junio de 2026.
- **Corrección de Bug (JWT Secret Fallback):** Se solucionó un error crítico (`Zero-length key is not supported`) al iniciar sesión. El archivo `server/_core/env.ts` inicializaba `cookieSecret` con un string vacío si la variable de entorno `JWT_SECRET` no estaba definida. Se agregó un fallback automático a `crypto.randomBytes(32).toString('hex')` para generar una clave criptográfica aleatoria segura al vuelo durante el arranque local.
- **Documentación de Despliegue:** Se actualizaron los documentos `README.md`, `INSTALACION_LOCAL.md` e `INSTALLATION_INSTRUCTIONS.md` para reflejar los pasos de uso del script de inicialización.
