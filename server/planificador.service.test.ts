...
describe('Planificador Service', () => {
  describe('savePlanificacionMasiva', () => {
    ...
    it('debe permitir solapamiento (enroques) si las fechas son distintas (no borra la base)', () => {
      // Setup initial data: Plan de 5 días
      db.prepare('INSERT INTO historial_turnos (legajo, id_turno, fecha_inicio, fecha_fin) VALUES (?, ?, ?, ?)').run('333', 1, '2026-06-08', '2026-06-12');
      
      // Intentamos meter un Enroque (1 día solo)
      const newAsignaciones = [
        { legajo: '333', id_turno: 3, fecha_inicio: '2026-06-10', fecha_fin: '2026-06-10', es_excepcional: 0 }
      ];

      savePlanificacionMasiva(newAsignaciones);

      const turnos = db.prepare('SELECT * FROM historial_turnos WHERE legajo = ? ORDER BY fecha_inicio ASC, fecha_fin ASC').all('333') as any[];
      
      // Deben convivir el plan maestro y la excepción!
      expect(turnos.length).toBe(2);
      expect(turnos[0].id_turno).toBe(1); // El de 5 dias
      expect(turnos[1].id_turno).toBe(3); // El de 1 dia (Enroque)
    });
  });
});
